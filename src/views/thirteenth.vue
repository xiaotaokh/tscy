  ...
  <div class="thirteenth">
    <div class="search">
      <el-form :inline="true" :model="searchData" size="small" class="demo-form-inline">
        ...
        <el-form-item label="有无创业经历">
          <el-input @keyup.enter.native="onSubmit" v-model="searchData.cyjl" placeholder="有无创业经历"></el-input>
        </el-form-item>
        <el-form-item>
...
export default {
  ...
  methods: {
    ...
    onSubmit() {
      // 判断查询条件是否为空
      if (this.searchData.pname == "" && this.searchData.cyjl == "") {
        this.$message({
      ...
      let formData = {
        ...
        "cyjl":this.searchData.cyjl,
      };
      var self = this;
      axios.post('zfdtrTbMapper',qs.stringify(formData))
      .then(res=>{
...
　　　　　　const tHeader = ['个人编号IDA', '姓名',"性别",'身份证号码','出生日期','文化程度','政治面貌','身份类别','联系方式','有无创业经历','具体住址'];            // 对应表格输出的title
　　　　　　const filterVal = ['idp', 'pname',"sex",'idpf','birthday','degree','zzmm','sf','tel','cyjl','address'];                             // 对应表格输出的数据
　　　　　　const list = this.dataList;
　　　　　　const data = this.formatJson(filterVal, list);
　　　　　　export_json_to_excel(tHeader, data, '致富带头人信息Excel表');                   // 对应下载excel文件的名字
　　　　})
　　},
